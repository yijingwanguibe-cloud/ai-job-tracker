...
export default function InterviewPage() {
  ...
  const fetchInterviewData = async () => {
    ...
    try {
      ...
      setCurrentStep(1);
      console.log('\nStep 1/3: 提取关键词...');
      const keywords = await extractKeywords(currentJD, deepseekApiKey);

      setCurrentStep(2);
      console.log('\nStep 2/3: 匹配知识库...');
      const filteredKnowledgeBase = filterKnowledgeBase(notes, keywords);

      setCurrentStep(3);
      console.log('\nStep 3/3: 生成面试内容...');
      const result = await generateInterviewContent(currentJD, filteredKnowledgeBase, deepseekApiKey);
